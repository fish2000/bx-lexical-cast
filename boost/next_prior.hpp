...
//    const std::list<T>::iterator prev = bx::prior(p);
//    const std::list<T>::iterator next = bx::next(prev, 2);
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        bx::iterators::reverse_iterator< T > rx(x);
        bx::iterators::advance(rx, n);
        return rx.base();
    }
};
