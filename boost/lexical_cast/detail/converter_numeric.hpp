...
namespace bx { namespace detail {

template <class Source >
struct detect_precision_loss
{
    typedef Source source_type;
    typedef bx::numeric::Trunc<Source> Rounder;
    typedef BOOST_DEDUCED_TYPENAME mpl::if_<
        bx::is_arithmetic<Source>, Source, Source const&
    >::type argument_type ;
...
{
    inline bool operator() ( bx::numeric::range_check_result r ) const BOOST_NOEXCEPT {
        return (r == bx::numeric::cInRange);
    }
};

template <typename Target, typename Source>
inline bool noexcept_numeric_convert(const Source& arg, Target& result) BOOST_NOEXCEPT {
    typedef bx::numeric::converter<
            Target,
            Source,
            bx::numeric::conversion_traits<Target, Source >,
            nothrow_overflow_handler,
            detect_precision_loss<Source >
    > converter_orig_t;

    typedef BOOST_DEDUCED_TYPENAME bx::mpl::if_c<
        bx::is_base_of< detect_precision_loss<Source >, converter_orig_t >::value,
        converter_orig_t,
...
{
    static inline bool try_convert(const Source &arg, Target& result) BOOST_NOEXCEPT {
        typedef BOOST_DEDUCED_TYPENAME bx::mpl::eval_if_c<
                bx::is_float<Source>::value,
                bx::mpl::identity<Source>,
                bx::make_unsigned<Source>
        >::type usource_t;
 ...
 *
 *
 * Rule 2) required because bx::lexical_cast has the behavior of
...
struct dynamic_num_converter_impl
{
    static inline bool try_convert(const Source &arg, Target& result) BOOST_NOEXCEPT {
        typedef BOOST_DEDUCED_TYPENAME bx::mpl::if_c<
            bx::is_unsigned<Target>::value &&
            (bx::is_signed<Source>::value || bx::is_float<Source>::value) &&
            !(bx::is_same<Source, bool>::value) &&
            !(bx::is_same<Target, bool>::value),
